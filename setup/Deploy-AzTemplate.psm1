...
function New-AzDeployment2 {
    ...
    $TemplateFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($ExecutionPath, $TemplateFile))
    $TemplateParametersFile = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($ExecutionPath, $TemplateParametersFile))
...
#                                  | Select-Object -Expand '_artifactsLocation' -ErrorAction Ignore
    $UsingTemplateLink = $($TemplateJson | Out-String) -like '*deployment().properties.templateLink.uri*'

    #if the switch is set or the standard parameter is present in the template, upload all artifacts
    if ($UploadArtifacts -Or $UsingTemplateLink) {
        # Convert relative paths to absolute paths if needed
        $ArtifactStagingDirectory = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($ExecutionPath, $ArtifactStagingDirectory))
        $DSCSourceFolder = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($ExecutionPath, $DSCSourceFolder))
...
#>
        ...
        New-AzStorageContainer -Name $StorageContainerName -Context $StorageAccount.Context -Permission Container -ErrorAction SilentlyContinue *>&1
        ...
        foreach ($SourcePath in $ArtifactFilePaths) {
            ...
            if ($SourcePath -like "$DSCSourceFolder*" -and $SourcePath -like "*.zip" -or !($SourcePath -like "$DSCSourceFolder*")) {
                ...
                Set-AzStorageBlobContent -File $SourcePath -Blob $blobName -Container $StorageContainerName -Context $StorageAccount.Context -Force 1>$null
            }
        }
